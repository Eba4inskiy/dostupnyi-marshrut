"""Maintenance: import the seven right-bank Kyiv districts from a full Ukraine PBF.
pip install osmium shapely; python scripts/import-right-bank.py /path/ukraine-latest.osm.pbf
...
DISTRICTS = {1754513, 1754514, 1754751, 1754928, 1754975, 1755013, 1755014}
...
class Extract(osmium.SimpleHandler):
    def __init__(self, borders):
        super().__init__(); self.nodes = {}; self.ways = {}; self.boundaries = {}; self.border_ids = borders.way_ids
    def node(self, n):
...
def main():
    ...
    if len(borders.relations) != 7: raise RuntimeError('All seven district relations are required')
    ...
    for way in ways:
        ...
        if len(coords) != len(way['nodes']): continue
    ...
    for (row, col), tile_ways in sorted(cells.items()):
        ...
        data = {'elements': tile_ways + [nodes[i] for i in sorted(ids)], 'osm3s': {'timestamp_osm_base': timestamp},
                'prototype_meta': {'bounds': box, 'retrieved_at': retrieved, 'scope': 'kyiv-right-bank'}}
        write(out/filename, data)
        index.append({'url': f'/data/network/{filename}', 'bounds': box, 'bytes': (out/filename).stat().st_size})
    write(ROOT/'lib/right-bank.json', {'type': 'FeatureCollection', 'features': regions})
    ...
    meta = {'bounds': bounds, 'retrieved_at': retrieved, 'scope': 'kyiv-right-bank'}
