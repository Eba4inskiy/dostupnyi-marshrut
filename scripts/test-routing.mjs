...
const {inRightBank,districtAt}=await import(pathToFileURL(join(temp,"region.mjs")).href);
...
test("coverage includes all seven right-bank districts and excludes the left bank",()=>{
 const districts=new Set(PLACES.map(p=>districtAt(p)).filter(Boolean));assert.equal(districts.size,7);
 for(const p of PLACES)assert.ok(inRightBank(p),p.label);
 assert.equal(inRightBank({lat:50.39766,lng:30.63315}),false);
 assert.equal(inRightBank({lat:50.5,lng:30.2}),false);
 assert.equal(inRightBank({lat:NaN,lng:30.5}),false);
});
test("bundled municipal snapshots retain source identity, dates, and right-bank geometry",async()=>{
 for(const id of ["bicycle","dogs","toilets"]){
  ...
  for(const f of d.features){assert.ok(f.source);assert.ok(inRightBank(f));assert.equal(f.confirmations,0);for(const line of f.lines||[])for(const[lat,lng]of line)assert.ok(inRightBank({lat,lng}));}
 }
});
test("complete extract has no missing way nodes or seams and routes beyond the centre",async()=>{
 const index=JSON.parse(await readFile("public/data/network-index.json","utf8")),elements=new Map();assert.equal(index.districts.length,7);
