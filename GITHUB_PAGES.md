...
У проєкті є окрема статична збірка для GitHub Pages. Вона підтримує карту всіх десяти районів Києва, п’ять профілів, геолокацію, побудову маршрутів у браузері, фільтри, міські відкриті дані та світлу й темну теми. Основні функції працюють без ключів API. Додатковий пошук місць Google Maps потребує власного ключа — [як підключити](GOOGLE_MAPS.md).
...
5. Відкрийте посилання із завдання `deploy` або кнопки **Visit site** у Settings → Pages. Ознаки цього оновлення: новий логотип і напис **«Увесь Київ · 10 районів»** у панелі планування.
...
### Додатково: Google Maps

Налаштуйте Google Cloud за [GOOGLE_MAPS.md](GOOGLE_MAPS.md), додайте **Settings → Secrets and variables → Actions → New repository secret** з назвою **`VITE_GOOGLE_MAPS_API_KEY`** і значенням вашого браузерного ключа. Повторіть **Deploy GitHub Pages**. Workflow вже підключає його під час збірки; ключ має обмеження на домен сайту та потрібні API. Без секрету сайт працює, але пошук Google всередині застосунку недоступний.
